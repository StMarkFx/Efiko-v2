...
class SupabaseDB:
    ...
    def verify_setup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            vector_ext = self.client.table('pg_extension').select('*').eq('extname', 'vector').execute()
            vector_enabled = len(vector_ext.data) > 0
            
            # Check if tables exist
            tables = self.client.table('information_schema.tables').select('table_name').eq('table_schema', 'public').execute()
            ...
            functions = self.client.table('information_schema.routines').select('routine_name').eq('routine_schema', 'public').execute()
    ...
    def add_document(self, user_id: str, title: str, content: str, source_type: str, source_url: Optional[str] = None) -> Dict[str, Any]:
        try:
            ...
            doc_response = self.client.table('documents').insert(document_data).execute()
            ...
            self.client.table('user_documents').insert(user_doc_data).execute()
